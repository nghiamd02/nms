...
class _NoteScreenState extends State<NoteScreen> {
  ...
  Widget _addingDialog({Map<String, dynamic>? currentNote}){
    if(currentNote != null){
      ...
      _categoryValue = _categories.firstWhere((element) => element[columnCategoryId] == currentNote[columnCategory])[columnCategoryName];
      _priorityValue = _priorities.firstWhere((element) => element[columnPriorityId] == currentNote[columnPriority])[columnPriorityName];
      _statusValue = _statusList.firstWhere((element) => element[columnStatusId] == currentNote[columnStatus])[columnStatusName];
  ...
  Future<void> _updateNote(int id) async {
    await NoteHelper.updateNote(Note(
      ...
      category: _categories.firstWhere((element) => element[columnCategoryName] == _categoryValue)[columnCategoryId],
      priority: _priorities.firstWhere((element) => element[columnPriorityName]  == _priorityValue)[columnPriorityId],
      status: _statusList.firstWhere((element) => element[columnStatusName] == _statusValue)[columnStatusId],
      planDate: DateFormat('dd/MM/yyyy').format(_planDate!),
      createAt: getCurrentDateTime(),
    ));
    _refreshJournals();
